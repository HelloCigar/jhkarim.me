...
function CommandDialogBackdrop({ className, ...props }: CommandDialogPrimitive.Backdrop.Props) {
  return (
    <CommandDialogPrimitive.Backdrop
      className={cn(
        'fixed inset-0 z-dropdown bg-black/32 backdrop-blur-sm transition-all duration-200 data-[ending-style]:opacity-0 data-[starting-style]:opacity-0',
        className
      )}
...
function CommandDialogViewport({ className, ...props }: CommandDialogPrimitive.Viewport.Props) {
  return (
    <CommandDialogPrimitive.Viewport
      className={cn(
        'fixed inset-0 z-dropdown flex flex-col items-center px-4 py-[max(1rem,4vh)] sm:py-[10vh]',
        className
      )}
    ...
    <CommandDialogPortal>
      ...
      <CommandDialogViewport>
        <CommandDialogPrimitive.Popup
          className={cn(
            '-translate-y-[calc(1.25rem*var(--nested-dialogs))] relative row-start-2 flex max-h-105 min-h-0 w-full min-w-0 max-w-xl scale-[calc(1-0.1*var(--nested-dialogs))] flex-col rounded-2xl border bg-base color-base opacity-[calc(1-0.1*var(--nested-dialogs))] shadow-lg outline-none transition-[scale,opacity,translate] duration-200 ease-in-out will-change-transform before:pointer-events-none before:absolute before:inset-0 before:rounded-[calc(var(--radius-2xl)-1px)] before:bg-#8881 before:shadow-[0_1px_rgba(0,0,0,0.04)] data-[nested]:data-[ending-style]:translate-y-8 data-[nested]:data-[starting-style]:translate-y-8 data-[nested-dialog-open]:origin-top data-[ending-style]:scale-98 data-[starting-style]:scale-98 data-[ending-style]:opacity-0 data-[starting-style]:opacity-0 **:data-[slot=scroll-area-viewport]:data-[has-overflow-y]:pe-1 dark:before:shadow-[0_-1px_rgba(255,255,255,0.06)]',
            className
          )}
...
function CommandPanel({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      className="-mx-px not-has-[+[data-slot=command-footer]]:-mb-px relative min-h-0 rounded-t-xl not-has-[+[data-slot=command-footer]]:rounded-b-2xl border border-b-0 bg-base bg-clip-padding shadow-xs [clip-path:inset(0_1px)] not-has-[+[data-slot=command-footer]]:[clip-path:inset(0_1px_1px_1px_round_0_0_calc(var(--radius-2xl)-1px)_calc(var(--radius-2xl)-1px))] before:pointer-events-none before:absolute before:inset-0 before:rounded-t-[calc(var(--radius-xl)-1px)] **:data-[slot=scroll-area-scrollbar]:mt-2"
      {...props}
    />
  )
}
...
function CommandShortcut({ className, ...props }: React.ComponentProps<'kbd'>) {
  return (
    <kbd
      className={cn(
        'ms-auto font-medium font-sans color-faint text-xs tracking-widest',
        className
      )}
...
function CommandFooter({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      className={cn(
        'flex items-center justify-between gap-2 rounded-b-[calc(var(--radius-2xl)-1px)] border-t px-5 py-3 color-muted text-xs',
        className
      )}
