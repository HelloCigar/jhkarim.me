  ...
  'flex shrink-0 items-center justify-center [&_svg]:pointer-events-none [&_svg]:shrink-0',
  {
    ...
    variants: {
      variant: {
        ...
        icon: "relative flex size-9 shrink-0 items-center justify-center rounded-md border bg-base color-base shadow-sm before:pointer-events-none before:absolute before:inset-0 before:rounded-[calc(var(--radius-md)-1px)] before:shadow-[0_1px_rgba(0,0,0,0.04)] dark:before:shadow-[0_-1px_rgba(255,255,255,0.06)] [&_svg:not([class*='size-'])]:size-4.5",
      },
    },
  }
)
...
function EmptyDescription({ className, ...props }: React.ComponentProps<'p'>) {
  return (
    <div
      className={cn(
        'color-muted text-sm [&>a:hover]:color-active [&>a]:underline [&>a]:underline-offset-4 [[data-slot=empty-title]+&]:mt-1',
        className
      )}
